  ...
  >
    ...
    <div class="room-card">
      <div class="flex items-start justify-between gap-4">
        <div class="flex-1 min-w-0">
          <label class="block text-sm text-white/50 mb-2">Current Story/Issue</label>
          ...
          <div v-if="isEditingIssue && isHost" class="flex gap-2">
            ...
            <button
              class="btn-primary"
              @click="handleSetIssue"
            >
              Save
            </button>
            <button
              class="btn-secondary"
              @click="isEditingIssue = false"
            >
              Cancel
            </button>
          </div>
          ...
          >
            <p 
              ...
              :class="{ 'text-white/40 italic': !room.currentIssue }"
            >
    ...
    <div class="room-card">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-mono font-bold tracking-widest text-poker-gold">
            {{ room.id }}
          </p>
        </div>
        <button
    ...
    >
      <button
        ...
        :class="{ 'opacity-50 cursor-not-allowed': votedPlayers.length === 0 }"
        @click="revealVotes"
      >
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
